...
% updated from the github version: Range now use two parameters instead of
% one
mp_handle.get_output = @get_output;
mp_handle.get_para = @get_para;
mp_handle.fcn_output = sprintf('Use the GUI once, and run mp_handle.get_output to get the updated output;\n Use mp_handle.get_para to get the current parameter set');
control_width = 200;
control_height = 30;
...
for i_para = 1:n_para
    ...
    if ischar(cur_para{1})||length(cur_para{1})>1
        ...
        if ismember('Range',para_info)
            gen_newpos;
        RangeValue = cur_para([find(ismember(para_info,'Range'))+1+1,find(ismember(para_info,'Range'))+1+2]);
        ...
        if ismember('RangeWithStep',para_info)
            gen_newpos;
        RangeValue = cur_para([find(ismember(para_info,'Range'))+1+1,find(ismember(para_info,'Range'))+1+2]);
        StepSize = cur_para(find(ismember(para_info,'Range'))+1+3)
            Child.Slider = uicontrol('Style', 'slider',...
                'Min',RangeValue{1},'Max',RangeValue{2},'Value',cur_para{1},...
                'Position', new_pos,...
                'UserData',i_para,...
